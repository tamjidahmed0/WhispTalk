...
const Security = () => {
  ...
  return (
    ...
    <div className={`flex flex-col  mx-[15rem] mt-7 max-lg:mx-[5rem] max-md:mx-[2rem]`}>
      <h1 className={` font-bold ${darkMode ===true && 'text-white'}`}>Login & recovery</h1>
      <span className={` ${darkMode ===true && 'text-white'}`}>Manage your passwords, login preferences and recovery methods.</span>
      
 

    {options.map((value, index)=>(
     <Link href={`/t/settings/security?tab=${value.link}`} key={index} className={`${darkMode ===true ?'bg-[#161A20] hover:bg-[#242b35]' : 'hover:bg-blue-100'}  px-6 py-6 rounded-xl mt-6 shadow-lg  flex justify-between items-center  cursor-pointer`}>
